...
- **Shared team workspace (post-launch):** QR codes, folders, pages, media and analytics are visible to and editable/deletable by every signed-in teammate; `user_id` now only records the **creator** and `updated_by` the last editor (system templates are still read-only; backups still export what *you* created). Do not add `userId` filters to new queries. Handoff fields (`assigned_to`, `next_action`, `notes`, `checklist` jsonb) live on `qr_codes` and `page_templates` and are written only through `PUT /api/workflow/[kind]/[id]` (`src/lib/workflow/`), shown by `WorkflowDialog`. `GET /api/qr/[id]/check` reports what a scan would do (`src/lib/qr/status.ts`) plus a fetch of the destination (`check-destination.ts`: public http(s) only, redirects never followed) without counting a scan. Unified search is `src/lib/search/search.ts` (`/search`, `/api/search`, header box). Migration `0003` is additive.
- **Health, monitoring, history, print (v1.2.0):** `qr_destination_history` records every dynamic destination change (`src/lib/qr/destination-history.ts`; `POST /api/qr/[id]/restore` points a code back and is itself recorded). The daily cron `/api/cron/link-check` (`src/lib/monitoring/`) stores `health_status/message/checked_at` on `qr_codes` and logs system activity (`userId` null, shown as "Link check"); editing or restoring a destination clears the stored result. The health dialog decodes the code in the browser with `jsqr` (`scan-test.ts`), applies design rules (`design-checks.ts`) and merges everything in `health-verdict.ts`. Card builder real sizes: `CustomCardDesign.sizeMm/bleedMm` (canvas stays ~600 px on the long side, `resizeDesign` moves elements when the size changes; guides in `card-builder/print-guides.tsx`); the print dialog opens at the design's size. Print PDFs: geometry in `print-layout.ts`, artwork in `print-card.ts` (card renderers `renderCardRaster`/`renderCustomCardCanvas` return the canvas plus the QR position), PDF in `print-pdf.ts` (`pdf-lib`, loaded lazily). **DB pool:** keep `max` above the most queries any page sends at once (dashboard: 9); when queries queue on one connection Supabase's transaction pooler (6543) stalls. Vercel functions are pinned to `hnd1` in `vercel.json` to sit beside the database.
